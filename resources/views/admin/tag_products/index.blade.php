<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-row w-full justify-between items-center">
            ...
            <a href="{{ route('admin.tags.create') }}" class="inline-block rounded-md bg-indigo-600 px-4 py-2 text-center font-medium text-white hover:bg-indigo-700 transition duration-200">
                Add Tag    
            </a>
        </div>
    </x-slot>
            ...
            <ul role="list" class="divide-y divide-gray-200">
                ...
                <li class="flex justify-between items-center gap-x-6 py-5 px-4 hover:bg-gray-50">
                    <div class="flex items-center min-w-0 gap-x-4">
                        <img class="h-12 w-12 rounded object-cover bg-gray-50" src="{{ Storage::url($tag->icon) }}" alt="{{ $tag->name }}">
                ...
                @empty
                <p class="text-center text-gray-500 py-5">No Tag found.</p>
                @endforelse
            </ul>
        </div>
    </div>
