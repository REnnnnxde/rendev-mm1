<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Details | My Furniture</title>
    <link rel="shortcut icon" href="{{ asset('assets/svgs/logo-mark.svg')}}" type="image/x-icon">
    <link rel="stylesheet" href="{{ asset('css/main.css')}}">
    <link rel="stylesheet" href="https://unpkg.com/flickity@2/dist/flickity.min.css">
</head>

<body>
    <!-- Topbar -->
    <section class="relative flex items-center justify-between gap-5 wrapper">
        <a href="{{ route('front.index') }}" class="p-2 bg-white rounded-full">
            <img src="{{asset('assets/svgs/ic-arrow-left.svg')}}" class="size-5" alt="Back">
        </a>
        <p class="absolute text-base font-semibold translate-x-1/2 -translate-y-1/2 top-1/2 right-1/2">
            Details
        </p>
        <button type="button" class="p-2 bg-white rounded-full">
            <img src="{{asset('assets/svgs/ic-triple-dots.svg')}}" class="size-5" alt="Options">
        </button>
    </section>

    <!-- Product Image -->
    <img src="{{ Storage::url($product->photo) }}" class="h-[220px] w-auto mx-auto relative z-10" alt="{{ $product->name }}">

    <!-- Product Details Section -->
    <section class="bg-white rounded-t-[60px] pt-[60px] px-6 pb-5 -mt-9 flex flex-col gap-5 max-w-[425px] mx-auto">
        <div>
            <!-- Product Name and Category -->
            <div class="flex items-center justify-between">
                <div class="flex flex-col gap-1">
                    <span class="font-bold text-[22px]">
                        {{ $product->name }}
                    </span>
                    <hr class="border-t border-gray-600 my-2"> <!-- Horizontal line -->
                    <div class="flex items-center gap-2 mt-1">
                        <img src="{{ Storage::url($product->category->icon) }}" class="size-[30px]" alt="{{ $product->category->name }}">
                        <p class="font-semibold text-balance">
                            {{ $product->category->name }}
                        </p>
                    </div>
                </div>
            </div>

            <!-- Product Description -->
            <p class="mt-3.5 text-base leading-7">
                {{ $product->about }}
            </p>
        </div>

        <!-- Tags Section -->
        <div id="featureSlider" class="-mx-6">
            <!-- Popular Tags -->
            @forelse ($tags as $tag)
            <div class="w-[98px] border-none rounded-2xl p-3.5 flex flex-col gap-2.5 items-center justify-center mr-4">

                    @if ($tag->icon)
                        <img src="{{ Storage::url($tag->icon) }}" class="size-10" alt="{{ $tag->name }}">
                    @else
                        <p>No Icon</p>
                    @endif
                    <p class="text-sm font-semibold">
                        {{ $tag->name }}
                    </p>
                </div>
            @empty
                <p>Data Tag belum ada.</p>
            @endforelse
        </div>

        <!-- Price and Add to Cart -->
        <div class="flex items-center justify-between">
            <div class="flex flex-col gap-0.5">
                <p class="text-lg min-[350px]:text-2xl font-bold">
                    Rp{{ number_format($product->price, 0, '.', '.') }}
                </p>
                <p class="text-sm text-grey">
                    /quantity
                </p>
            </div>

            <!-- Add to Cart Form -->
            <form action="{{ route('carts.store', $product->id) }}" method="POST">
                @csrf
                <button type="submit" class="inline-flex w-max text-white font-bold text-base bg-primary rounded-full px-[30px] py-3 justify-center items-center whitespace-nowrap">
                    Add to Cart
                </button>
            </form>
        </div>
    </section>

    <!-- JS -->
    <script src="https://code.jquery.com/jquery-3.7.1.min.js" integrity="sha256-/JqT3SQfawRcv/BIHPThkBvs0OEvtFFmqPF/lYI/Cxo=" crossorigin="anonymous"></script>
    <script src="https://unpkg.com/flickity@2/dist/flickity.pkgd.min.js"></script>
    <script src="{{ asset('scripts/sliderConfig.js') }}" type="module"></script>

</body>

</html>
